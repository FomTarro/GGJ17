...
////// Specular:
                ...
                float2 node_3397 = ((_FlowMap_var.rgb.rg*2.0+-1.0)*(_FlowPower*(-1.0)));
                float4 node_2715 = _Time + _TimeEditor;
                float node_5143 = (node_2715.r*_FlowSpeed);
                float node_1147 = frac(node_5143);
                float2 node_1077 = (i.uv0+(node_3397*node_1147));
                float4 node_445 = tex2D(_Diffuse,TRANSFORM_TEX(node_1077, _Diffuse));
                float2 node_2246 = (i.uv0+(node_3397*frac((0.5+node_5143))));
                float4 node_4825 = tex2D(_Diffuse,TRANSFORM_TEX(node_2246, _Diffuse));
                float node_757 = 0.5;
                float3 node_550 = lerp(node_445.rgb,node_4825.rgb,abs(((node_757-node_1147)/node_757)));
                float node_4074 = 0.0;
                ...
                float3 diffuseColor = lerp(_FoamColor.rgb,lerp(_DepthsColor.rgb,(node_550*_Color.rgb),saturate((saturate((node_550*12.5+-11.5)).rgb.b+saturate((node_4074 + ( ((sceneZ-partZ) - node_4074) * ((clamp(_DepthMurkiness,3,100)*0.01) - node_4074) ) / (1.0 - node_4074)))))),round(saturate((sceneZ-partZ)/(saturate(sceneZ)*clamp(_FoamAmount,0,50)*sin(node_8728.rgb))))); // Need this for specular when using metallic
...
/// Final Color:
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : SV_Target {
                ...
                float2 node_4302 = (node_3846.r+i.uv0);
                float4 node_8728 = tex2D(_FoamNoise,TRANSFORM_TEX(node_4302, _FoamNoise));
                float3 diffColor = lerp(_FoamColor.rgb,lerp(_DepthsColor.rgb,(node_550*_Color.rgb),saturate((saturate((node_550*12.5+-11.5)).rgb.b+saturate((node_4074 + ( ((sceneZ-partZ) - node_4074) * ((clamp(_DepthMurkiness,3,100)*0.01) - node_4074) ) / (1.0 - node_4074)))))),round(saturate((sceneZ-partZ)/(saturate(sceneZ)*clamp(_FoamAmount,0,50)*sin(node_8728.rgb)))));
                float specularMonochrome;
                float3 specColor;
